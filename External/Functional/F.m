...
@implementation F static dispatch_queue_t F_queue;
...
+ (void) dontUseConcurrency { F_concurrently = NO; }

+ (void) useQueue:(dispatch_queue_t) queue {
  F_queue = queue;
}

+ (void) concurrently:(Blk) block {
  [self useConcurrency];
  block();
  [self dontUseConcurrency];
}

+ (void) concurrently:(Blk)block withQueue:(dispatch_queue_t) queue {
  [self useConcurrency];
  [self useQueue:queue];
  block();
  [self dontUseConcurrency];
...
+ (void) eachInArray:(NSArray *) arr withBlock:(ObjBlk) block {
  if (F_concurrently) {
...
+ (void) eachInArrayWithIndex:(NSArray *) arr withBlock:(ObjIntBlk) block {
  if (F_concurrently) {
    dispatch_apply([arr count], F_queue, ^(size_t i) {
...
+ (void) eachInDict:(NSDictionary *) dict withBlock:(ObjObjBlk) block {
  if (F_concurrently) {
...
+ (NSArray *) mapArray:(NSArray *) arr withBlock:(Obj_ObjBlk) block {
  ...
  if (F_concurrently) {
    ...
    for (id obj in arr) { id x = block(obj); if (x) [mutArr addObject:x];  }
  }
  return [NSArray arrayWithArray:mutArr];
}

+ (NSDictionary *) mapDict:(NSDictionary *) dict withBlock:(Obj_ObjObjBlk) block {
...
+ (NSArray *) filterArray:(NSArray *) arr withBlock:(Bool_ObjBlk) block {
...
+ (NSDictionary *) filterDictionary:(NSDictionary *) dict withBlock:(Bool_ObjObjBlk) block {
  NSMutableDictionary *mutDict = [NSMutableDictionary dictionaryWithCapacity:[[dict allKeys] count]];
  if (F_concurrently) {
    dispatch_semaphore_t itemLock = dispatch_semaphore_create(1);
    NSArray *keys = [dict allKeys];
...
+ (NSArray *) rejectArray:(NSArray *) arr withBlock:(Bool_ObjBlk) block {
  if (F_concurrently) {
    NSMutableArray *nilArray = [NSMutableArray arrayWithCapacity:[arr count]];
    for (int i=0; i < [arr count]; i++) {
      [nilArray addObject:[NSNull null]];
...
+ (NSDictionary *) rejectDictionary:(NSDictionary *) dict withBlock:(Bool_ObjObjBlk) block {
...
+ (BOOL) allInArray:(NSArray *) arr withBlock:(Bool_ObjBlk) block {
...
+ (BOOL) allInDictionary:(NSDictionary *) dict withBlock:(Bool_ObjObjBlk) block {
  __block BOOL validForAll = true;
...
+ (BOOL) anyInArray:(NSArray *) arr withBlock:(Bool_ObjBlk) block {
...
+ (BOOL) anyInDictionary:(NSDictionary *) dict withBlock:(Bool_ObjObjBlk) block {
  __block BOOL validForAny = false;
...
+ (NSNumber *) countInArray:(NSArray *) arr withBlock:(Bool_ObjBlk) block {
...
+ (NSNumber *) countInDictionary:(NSDictionary *) dict withBlock:(Bool_ObjObjBlk) block {
  __block NSInteger ctr = 0;
...
+ maxArray:(NSArray *) arr withBlock:(CompareArrayBlock) block {
...
+ maxDict:(NSDictionary *) dict withBlock:(CompareDictBlock) block {
...
+ minArray:(NSArray *) arr withBlock:(CompareArrayBlock) block {
  if ([arr count]<1) return NULL;
...
+ minDict:(NSDictionary *) dict withBlock:(CompareDictBlock) block {
  if ([dict count] < 1) return NULL;
...
+ (NSDictionary *) groupArray:(NSArray *) arr withBlock:(Obj_ObjBlk) block {
  NSMutableDictionary *mutDictOfMutArrays = @{}.mutableCopy;
  for (id obj in arr) {
  ...
  NSMutableDictionary *mutDictOfArrays = @{}.mutableCopy;
...
+ (NSArray *) dropFromArray:(NSArray *) arr whileBlock:(Bool_ObjBlk) block {
...
+ (void) times:(NSNumber*) nr RunBlock:(NumBlk)block { if (!block) return;
  for (int i = 0; i < [nr uIV]; i++) block(@(i));
}


+ (void) asynchronously:(Blk) block {
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), block);
}

+ (void) onUIThread:(Blk) block {
  if ([NSThread isMainThread]) block();
  else dispatch_sync(dispatch_get_main_queue(), block);
}

+ (NSArray *) mapRangeFrom:(NSInteger) from To:(NSInteger) to withBlock:(Obj_IntBlk) block {
